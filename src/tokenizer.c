...
char **split_command_line(const char *line, int *argc) {
  ...
  enum { NORMAL, IN_SQUOTE, IN_DQUOTE } state = NORMAL;
  ...
  while (*p) {
    ...
    if (state == NORMAL) {
      if (c == ' ' || c == '\t') {
        ...
        /* Special case: backslash followed by double quote opens a quoted block */
        if (*(p + 1) == '"') {
          p++; /* skip backslash */
          state = IN_DQUOTE;
        } else {
